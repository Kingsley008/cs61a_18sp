    ...
    def eval_helper(expr):
        ...
        if scheme_symbolp(first) and first in SPECIAL_FORMS:
            ...
            if isinstance(expr, Pair):
                ...
                first, rest = expr.first, expr.second
                args = expr.second.map(eval_helper)
                # evaluate operator
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        # match argument and formal parameter
        f = formals
        v = vals
        ...
        if f == nil and v == nil:
            return child
        elif f == nil and v != nil:
            raise SchemeError
        elif f != nil and v == nil:
            raise SchemeError
        count_for = get_num(formals)
        count_val = get_num(vals)
        # if it does not match, raise Error
        if count_for != count_val:
            raise SchemeError
        # do the Binding
        while True:
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            # 使用*arg将args传递给函数将使python解压缩arg中的值并将其传递给函数
            return self.fn(*python_args)
        except TypeError:
            raise SchemeError('{0} can not apply {1}'.format(python_args, self.fn))
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        if isinstance(rest.first, Pair):
            val = scheme_eval(rest.first, env)
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        # Convert shorthand to lambda expr
        rest = expressions.second
        lam = LambdaProcedure(target.second, rest, env)
...
def do_quote_form(expressions, env):
    ...
    #  read_line(" '(a b) ") -> Pair('quote', Pair(Pair('a', Pair('b', nil)), nil))
    ...
    while True:
        if scheme_truep(scheme_eval(expressions.first, env)):
            if expressions.second == nil:
                # deal with define
                try:
                    if expressions.first.first == 'define':
                        return expressions.first.second.first
                    return scheme_eval(expressions.first, env)
...
def make_let_frame(bindings, env):
    ...
    formals_ls = []
    val_ls = []
    # separate formal and val
    while bindings != nil:
        check_form(bindings.first, 2, 2)
        # (x 42)
        cur_exp = bindings.first
        if isinstance(cur_exp.first, int):
            raise SchemeError
        formals_ls.append(cur_exp.first)
        val_ls.append(eval_all(cur_exp.second, env))
        bindings = bindings.second

    # construct python list to scheme list
    formals_str = formals_ls.__str__()
    val_str = val_ls.__str__()
    s = formals_str.translate(formals_str.maketrans({'[': '(', ']': ')', '\'': '', ',': ' '}))
    formals = read_line(s)
    val = read_line(val_str.translate(val_str.maketrans({'[': '(', ']': ')', '\'': '', ',': ' '})))
    check_formals(bindings)
