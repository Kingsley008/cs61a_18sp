...
def scheme_read(src):
    ...
    if val == 'nil':
        ...
        # deal not quote '('
        if str(cur) != '(' and str(cur) != "'":
            src.remove_front()
            return read_line('(quote ' + str(cur) + ')')
        # deal quote '('
        count_left = 0
        for ls in src.lines:
            for item in ls[src.index:]:
                st = st + ' ' + str(item)
                if str(item) == "'":
                    src.remove_front()
                elif str(item) == '(':
        ...
        if src.current() is None:
            ...
            if src.current() == '(':
                ...
                if src.current() == ')':
                    ...
                    raise SyntaxError('pair only has 2 arguments')
            else:
                var = src.current()
                src.remove_front()
                if src.current() == ')':
            ...
            # BEGIN PROBLEM 1
            "*** YOUR CODE HERE ***"
            first = scheme_read(src)  # car (quote(a))
            rest = read_tail(src)   # '(a)   return readline(quote(a))
            return Pair(first, rest)
            # END PROBLEM 1
    except EOFError:
        raise SyntaxError('unexpected end of file')
