...
class Ant(Insect):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        if self.container and other.container == False and self.ant is None:
            return True
        else:
            return False
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0:
            ...
            for b in bee_list:
                armor_remain = b.armor - self.damage
                b.armor = armor_remain
            self.place.bees = [bee for bee in bee_list if bee.armor > 0]
            self.place.remove_insect(self)
        # END Problem 5
...
class LongThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        ...
        current_place = self.place
        place_count = 1
        while len(current_place.bees) == 0 or place_count <= self.min_range:
            current_place = current_place.entrance
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        ...
        alive_bees = [bee for bee in current_bees if bee.armor > 0]
...
class HungryAnt(Ant):
    ...
    implemented = True  # Change to True to view in the GUI
    food_cost = 4
    time_to_digest = 3  # spend 3 turns digesting before eating again
    ...
    def action(self, colony):
        ...
        if len(self.place.bees) < 1:
            ...
            if len(self.place.bees) > 1:
                random_num = random.randint(0, len(self.place.bees) - 1)
            else:
                random_num = 0
            self.eat_bee(self.place.bees[random_num])
        # END Problem 6
        ...
        if self.is_impostor:
            ...
            while current_place is not None:
                if current_place.ant is not None:
                    ...
                    if current_place.ant.ant is not None and not current_place.ant.ant.is_buffed:
                        current_place.ant.ant.damage = current_place.ant.ant.damage * 2
                        current_place.ant.ant.is_buffed = True
...
def make_slow(action):
    ...
    def new_action(colony):
        if colony.time % 2 == 0:
            action(colony)
    return lambda colony: new_action(colony)
    # END Problem EC
